...
//       }}
//     >
//       <div className="bg-white/20 backdrop-blur-lg rounded-xl shadow-2xl p-8 w-full max-w-md text-white">
...
export default function QuizSetup() {
  ...
  const handleStart = () => {
    const settings = { category, type, difficulty, timeLimit };
    localStorage.setItem("quizSettings", JSON.stringify(settings));
    navigate("/quizplay", { state: settings });
  };
    ...
    >
      <div className="bg-white/20 backdrop-blur-lg rounded-xl shadow-2xl p-8 w-full max-w-md text-white">
        ...
        <div className="mb-6">
          ...
          <div className="flex items-center space-x-4">
            <button
              className={`w-1/2 py-2 rounded-lg border ${timeLimit ? "bg-green-500" : "bg-white/10"}`}
              onClick={() => setTimeLimit(true)}
            >
              Yes
            </button>
            <button
              className={`w-1/2 py-2 rounded-lg border ${!timeLimit ? "bg-red-500" : "bg-white/10"}`}
        ...
        <motion.button
          whileHover={{ scale: 1.05 }}
          whileTap={{ scale: 0.95 }}
          className="w-full py-3 bg-white text-purple-700 font-bold rounded-xl shadow-xl"
          onClick={handleStart}
        >
          🚀 Start Quiz
        </motion.button>
